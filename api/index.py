# api/index.py — ПРОСТАЯ ВЕРСИЯ без админки и фото
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family: system-ui, sans-serif; background: #0f172a; color: #e2e8f0; margin: 0; padding: 16px; min-height: 100vh; }
        ...
        .tab { flex: 1; padding: 10px; text-align: center; border-radius: 6px; cursor: pointer; color: #94a3b8; }
        ...
        .inventory-item { margin-bottom: 12px; }
    </style>
</head>
<body>
    <div id="error" class="error" style="display: none;"></div>
    
    <div class="user-info">👤 ID: <span id="user-id">-</span></div>
    <h1 style="color: #a78bfa; margin: 0 0 20px 0; font-size: 24px;">Мои брони</h1>

    <div class="catalog-btn" onclick="openCatalog()">📦 Каталог товаров</div>
                    ...
                    '<div class="card inventory-item">' +
                        '<div style="display: flex; justify-content: space-between; align-items: start;">' +
                            '<div>' +
                                ...
                                '<div style="font-size: 14px; color: #34d399;">' + item.price_per_hour + '₸/час • ' + item.price_per_day + '₸/день</div>' +
                            '</div>' +
                            '<div style="text-align: right;">' +
                                '<div style="font-size: 20px; font-weight: bold; color: ' + (item.available_quantity > 0 ? '#34d399' : '#ef4444') + ';">' + item.available_quantity + '</div>' +
                                '<div style="font-size: 11px; color: #64748b;">из ' + item.total_quantity + '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>'
                ).join('');
